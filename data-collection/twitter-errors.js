...
class TwitterApiError extends Error {
  ...
  constructor(endpoint, errors, type = "Unknown") {
    super(
      `[${type}]: Twitter API Error; endpoint=${endpoint}; error=${JSON.stringify(
        errors[0]
      )}`
    );
    this.errors = errors;
 ...
 * For example, not allowed to view a protected tweet or DM a user.
 */
class UserNotFound extends TwitterApiError {
  constructor(endpoint, errors) {
    super(endpoint, errors, `UserNotFound`);
...
const wrapTwitterErrors = (endpoint, response) => {
  ...
  switch (errors[0].code) {
    ...
    case codes.COULDNT_FIND_USER:
      throw new UserNotFound(endpoint, errors);
